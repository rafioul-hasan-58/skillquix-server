...
export interface ITemp5Education {
  school?: string;
  institution?: string;
  period?: string;
}

export interface ITemp5Experience {
  title?: string;
  company?: string;
  period?: string;
  bullets?: string[];
...
export interface ITemp5ResumeData {
  name?: string;        // last name — displayed large
  firstName?: string;   // first name — displayed medium
  title?: string;       // job title — displayed small at top
  profileImage?: string;
  summary?: string;
  phone?: string;
  email?: string;
  ...
  website?: string;
  skills?: string[];
  education?: ITemp5Education[];
  experience?: ITemp5Experience[];
}
...
function contactRow(iconSvg: string, text: string): string {
  return `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
      ...
      <span style="font-family:${F};font-size:12px;color:${BODY};line-height:1.4;">${text}</span>
    </div>
  `;
}
...
const addressIcon = `<svg style="width:11px;height:11px;" fill="none" stroke="${WHITE}" viewBox="0 0 24 24"><circle cx="12" cy="10" r="3" stroke-width="2.2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.2" d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/></svg>`;
...
export const generateTemp5Html = (data: ITemp5ResumeData): string => {
  ...
  const photoHtml = data.profileImage
    ? `<img src="${data.profileImage}" alt="${data.firstName ?? "Profile"}" style="width:170px;height:100%;object-fit:cover;object-position:top;display:block;position:absolute;right:0;top:0;z-index:2;" />`
    : "";
    ...
    data.phone || data.email || data.address || data.website
      ? `<div style="margin-bottom:24px;">
          ...
          <div style="padding-left:4px;padding-top:8px;">
            ...
            ${data.address  ? contactRow(addressIcon, data.address) : ""}
            ${data.website  ? contactRow(websiteIcon, data.website) : ""}
          </div>
        </div>`
      : "";
  ...
  const skillsHtml =
    data.skills && data.skills.length > 0
      ? `<div style="margin-bottom:24px;">
          ...
          <div style="padding-left:4px;padding-top:8px;">
            <p style="font-family:${F};font-size:11px;font-weight:700;letter-spacing:0.10em;color:${BODY};text-transform:uppercase;margin:0 0 8px 0;">Professional</p>
            ${data.skills
              .map(
                (s) => `
              <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:7px;">
                ...
                <span style="font-family:${F};font-size:12px;color:${BODY};line-height:1.5;">${s}</span>
              </div>`
              )
              .join("")}
          </div>
      ...
      ? `<div>
          ...
          <div style="padding-left:4px;padding-top:8px;">
            ${data.education
              .map(
                (e, i) => `
              <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:${i < (data.education?.length ?? 1) - 1 ? "16px" : "0"};">
                ${dot}
                <div>
                  ${e.school ? `<p style="font-family:${F};font-size:11px;font-weight:700;letter-spacing:0.07em;color:${BODY};text-transform:uppercase;margin:0 0 2px 0;">${e.school}</p>` : ""}
                  ${e.institution ? `<p style="font-family:${F};font-size:12px;color:${BODY};margin:0 0 1px 0;">${e.institution}</p>` : ""}
                  ${e.period ? `<p style="font-family:${F};font-size:11px;color:${GRAY};margin:0;">${e.period}</p>` : ""}
                </div>
              </div>`
              )
              .join("")}
  ...
  const summaryHtml = data.summary
    ? `<div style="margin-bottom:28px;">
        ...
        <div style="border-left:3px solid ${STEEL};padding-left:14px;margin-top:10px;">
          <p style="font-family:${F};font-size:12px;color:${BODY};line-height:1.75;margin:0;">${data.summary}</p>
        </div>
      </div>`
    : "";
  ...
  const experienceHtml =
    data.experience && data.experience.length > 0
      ? `<div>
          ${rightSectionHeader("Working Experience")}
          <div style="margin-top:10px;">
            ${data.experience
              .map(
                (exp, i) => `
              <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:${i < (data.experience?.length ?? 1) - 1 ? "20px" : "0"};">
                ${dot}
                <div style="flex:1;">
                  ${exp.title ? `<p style="font-family:${F};font-size:12px;font-weight:700;letter-spacing:0.06em;color:${BODY};text-transform:uppercase;margin:0 0 3px 0;">${exp.title}</p>` : ""}
                  ${
                    exp.company || exp.period
                      ? `<p style="font-family:${F};font-size:12px;color:${BODY};margin:0 0 7px 0;">
                          ${exp.company ?? ""}
                          ${exp.company && exp.period ? `<span style="margin:0 7px;color:#9ca3af;">|</span>` : ""}
                          ${exp.period ? `<strong style="font-weight:700;">${exp.period}</strong>` : ""}
                        </p>`
                      : ""
                  }
                  ${
...
<head>
  ...
  <title>${[data.firstName, data.name].filter(Boolean).join(" ") || "CV"} – Resume</title>
  <style>
  ...
  <div class="cv-card">
    ...
    <div class="cv-header">
      ...
      <div class="name-block">
        ${data.title     ? `<p class="name-jobtitle">${data.title}</p>`  : ""}
        ${data.firstName ? `<p class="name-first">${data.firstName}</p>` : ""}
        ${data.name      ? `<p class="name-last">${data.name}</p>`       : ""}
      </div>
    ...
    <div class="cv-body">
      ...
      <div class="cv-right">
        ${summaryHtml}
        ${experienceHtml}
        <!-- Bottom-right steel bar decoration -->
